...
in accuracy simultaneously. These promising results validate our adaptation
approach automatically learning better policies and the use of adaptation at
runtime.
...
profiling efficiency. We use Bayesian Optimization to avoid exhaustively
exploring the large parameter space. BO significantly reduces the number of
samples needed, by more than 50$\times$. We analyze the characteristics of the
profile and observe that we only need to measure the processing times for
Pareto-optimal configurations. By using profile transfer, we eliminate the need
